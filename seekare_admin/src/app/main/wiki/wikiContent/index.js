...
const WikiContent = forwardRef((prop, ref) => {
  ...
  const categories =
    ...
    JSON.parse(window.localStorage.getItem("wikiSel"))?.categories;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isCreateWiki, activeWikiId, wikiSel]);
        ...
        {!isCreateWiki && (
          <Fragment>
            ...
            <Box className={classes.questionsWrapper} pt={2}>
              {getQuizeContent().map(({ title, cagId, quiz }) => {
                return (
                  <Box className={classes.questionWrapper} key={cagId}>
                    {/* <Collapse
                  header={
                    <Box className={classes.accordionHeader}>
                      <Box className={classes.accordionHeaderTitle}>
                        {title} ({quiz?.length || 0})
                      </Box>
                      {(isMd || isAdmin) && (
                        <IconButton
                  ...
                  classes={classes.accordionHeader}
                  expanded={expandedQuestion}
                  onChangePanel={handleChangeQuestionCollapse}
                  panelKey={`categories_${cagId}`}
                  key={cagId}
                  statusIcon={ExpandMoreIcon}
                > */}
                    <Box className={classes.accordionHeader}>
                      <Box className={classes.accordionHeaderTitle}>
                        {title} ({quiz?.length || 0})
                      ...
                      <IconButton
                        ...
                        onFocus={(event) => event.stopPropagation()}
                        disabled={quiz?.length == 0 && !isMd && !isAdmin}
                      >
                      </IconButton>
                    </Box>
                    {/* new question */}
                    {activeQuiz[cagId] && activeQuiz[cagId].id === "new" && (
                      <WikiQuizForm
                        onUpdateQuiz={(data) => handleAddNewQuiz(cagId, data)}
                        onCancelQuiz={() => handleCancelQuiz(cagId)}
                        ref={ref}
                        qTitle={qTitle}
                        qContent={qContent}
                      ...
                      ? quiz.map(({ _id: id, title, content }) =>
                          activeQuiz[cagId] && isEditWiki ? (
                            <WikiQuizForm
                              key={`qItm-${id}`}
                              onUpdateQuiz={(data) =>
                                handleUpdateQuiz(cagId, { ...data, id })
                              }
                              ref={ref}
                              onCancelQuiz={() => handleCancelQuiz(cagId)}
                              qTitle={qTitle}
