...
const LoginModalContent = ({ state = null, state_data }) => {
  ...
  const { handleSubmit, values, handleChange, errors, touched } = useFormik({
    ...
    onSubmit: async (values, actions) => {
      ...
      const res = await dispatch(signIn(values.emailOrName.toLowerCase(), values.password));
      await tocVisitorGetter(res.userID)
      if (status) {
  ...
  return (
    <Box className={classes.root}>
      ...
      <div className={classes.donthave}>
        <div className={classes.link} onClick={openRegisterModal}>
          New to SeeKare? Sign Up!
        </div>
      </div>
      {isSubmitting && <Loader />}
    </Box>
